...
let ranking = [
  ...
  { name: "Sanji", score: 8 },
  { name: "Nami", score: 23 },
  { name: "Chopper", score: 18 },
];

//Variables usando el método getElementById() para seleccionarlos del HTML
const phraseWelcome = document.getElementById("phrase");
const welcomeForm = document.getElementById("welcome");
const userScreen = document.getElementById("usernamescreen");
...
const endbutton = document.getElementById("theEnd");
const definition = document.getElementById("question");
const answerscreen = document.getElementById("answer");
const score = document.getElementById("pointsShow");
const temp = document.getElementById("seconds");
...
welcomeForm.addEventListener("submit", function (event) {
  ...
  if (userName === "" || !isNaN(userName)) {
    phraseWelcome.innerHTML = "Este nombre no es correcto.";
  }
  phraseWelcome.innerHTML = `¡Bienvenid@ al Pasapalabra Skylab, ${userName}!`;
  showRanking.style.visibility = "hidden";
  userScreen.style.visibility = "hidden";
  submitButton.style.visibility = "hidden";
  rulesBlock.style.visibility = "visible";
  gameBlock.style.visibility = "visible";
});

//Función para crear rosco
function crearRosco() {
...
//Función para mostrar preguntas usando el innerHTML
function showQuestion() {
  if (rosco[numQuestion].status === 0) {
    definition.innerHTML = rosco[numQuestion].question;
  } else {
    nextQuestion();
  }
}

//Función para la cuenta atrás donde se dice cuándo usar el cronómetro (únicamente cuando el contador sea mayor de 0, si no es así se finaliza el juego)
function crono() {
  temp.innerHTML = totalTime;
  if (totalTime === 0) {
    ...
    totalTime -= 1;
    timer = setTimeout(crono, 1500);
  }
}

//Evento para el botón de JUGAR donde se ejecutan la funcitón de crear rosco, se muestra la pregunta y se inicia el cronómetro
playbutton.addEventListener("click", function () {
  welcomeForm.style.visibility = "hidden";
  playbutton.style.visibility = "hidden";
  sendAnswer.style.visibility = "visible";
...
function nextQuestion() {
  ...
  //Si se han contestado todas las preguntas que el juego termine
  if (anwseredQuestions === letters.length) {
    return endGame();
  }
  //Si se ha completado el rosco se reinicia el número de preguntas
  if (numQuestion === rosco.length) {
    numQuestion = 0;
  }
  answerscreen.value = "";
  showQuestion();
...
//Evento para botón de enviar donde se "checkea" si la respuesta es correcta o no
sendAnswer.addEventListener("click", function () {
  userAnswer = document.getElementById("answer").value.toLocaleLowerCase();
  //Si el estatus es 0 se sigue ejecutando el código
  if (rosco[numQuestion].status === 0) {
    if (userAnswer === rosco[numQuestion].answer) {
      correctAnswers++;
      anwseredQuestions++;
      rosco[numQuestion].status = 1;
      score.innerHTML = correctAnswers;
      document.getElementsByClassName(
        rosco[numQuestion].letter
      )[0].style.background = "green";
    } else {
      incorrectAnswers++;
      anwseredQuestions++;
      rosco[numQuestion].status = 2;
      document.getElementsByClassName(
        rosco[numQuestion].letter
...
//Evento para el botón PASAPALABRA donde se ejecuta la función de siguiente pregunta
...
//Evento para el botón de REINICIAR donde se reinicia todo el juego
...
function printRanking() {
  ...
  ranking.push({ name: userName, score: correctAnswers });
  ...
  if (finalRanking.length !== 0) {
    alert("El ranking se encuentra así:");
    //Se usa el forEach para que se vaya imprimiendo cada elemento del array ranking
    finalRanking.forEach(function (playerResults) {
      const player = document.createElement("li");
      player.className = "ranking";
      player.textContent = `${playerResults.name}: ${playerResults.score}`;
      rankingTable.append(player);
    });
  }
}
...
function endGame() {
  ...
  sendAnswer.style.visibility = "hidden";
  pasapalabrabutton.style.visibility = "hidden";
  playAgainbutton.style.visibility = "visible";
